 ...
 * @brief Settings/Status Provider 的认证 HTTP 映射与有界 JSON 编解码
 */
...
static bool web_console_provider_find_field(const web_console_settings_provider_t *provider,
                                            const char *field_id,
                                            size_t *out_index)
{
    for (size_t index = 0U; index < provider->field_count; ++index)
    {
        if (strcmp(provider->fields[index].id, field_id) == 0)
        {
            *out_index = index;
            return true;
        }
    ...
    web_console_settings_update_t *out_update)
{
    ...
    for (int change_index = 0; change_index < change_count; ++change_index)
    {
        ...
        if (!web_console_provider_find_field(provider, id->valuestring, &field_index)
