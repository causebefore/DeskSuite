...
static esp_err_t rollback_wifi_init(esp_err_t error, bool wifi_handler_registered)
{
    if (wifi_handler_registered)
    {
        const esp_err_t cleanup_err =
            esp_event_handler_instance_unregister(WIFI_EVENT, ESP_EVENT_ANY_ID, s_runtime.wifi_event_instance);
...
{
    taskENTER_CRITICAL(&s_runtime.lifecycle_lock);
    const bool accepts = s_runtime.initialized && !s_runtime.deinitializing && !s_runtime.cleanup_failed;
    taskEXIT_CRITICAL(&s_runtime.lifecycle_lock);
    return accepts;
}
...
static void handle_disconnected(const void *event_data)
{
    ...
    {
        copy_ssid(event.link.ssid, sizeof(event.link.ssid), disconnected->ssid, disconnected->ssid_len);
...
{
    ESP_RETURN_ON_FALSE(config != nullptr && config->ssid[0] != '\0', ESP_ERR_INVALID_ARG, TAG, "STA 配置无效");
    ESP_RETURN_ON_ERROR(connect_init(), TAG, "初始化连接能力失败");
    ESP_RETURN_ON_FALSE(connect_internal_accepts_operations(), ESP_ERR_INVALID_STATE, TAG, "连接组件正在停止");
    ...
    utils_copy_string(reinterpret_cast<char *>(wifi_config.sta.ssid), sizeof(wifi_config.sta.ssid), config->ssid);
...
{
    ESP_RETURN_ON_FALSE(config != nullptr && config->ssid[0] != '\0', ESP_ERR_INVALID_ARG, TAG, "STA 候选配置无效");
    ESP_RETURN_ON_ERROR(connect_init(), TAG, "初始化连接能力失败");
    ESP_RETURN_ON_FALSE(connect_internal_accepts_operations(), ESP_ERR_INVALID_STATE, TAG, "连接组件正在停止");
    ESP_RETURN_ON_ERROR(connect_internal_ensure_sta_netif(), TAG, "创建 STA netif 失败");
    ...
    ESP_RETURN_ON_ERROR(esp_wifi_set_config(WIFI_IF_STA, &wifi_config), TAG, "设置 STA 候选配置失败");
    ESP_RETURN_ON_ERROR(connect_internal_wifi_start_once(), TAG, "启动 Wi-Fi 失败");
...
esp_err_t connect_get_link_snapshot_copy(connect_link_info_t *out)
{
    ...
    if (ap_err == ESP_ERR_WIFI_NOT_CONNECT || ap_err == ESP_ERR_WIFI_NOT_STARTED || ap_err == ESP_ERR_WIFI_CONN)
    {
        return ESP_OK;
    }
    if (ap_err != ESP_OK)
...
esp_err_t connect_deinit(void)
{
    ...
    {
        const esp_err_t err =
            esp_event_handler_instance_unregister(IP_EVENT, IP_EVENT_STA_GOT_IP, s_runtime.ip_event_instance);
