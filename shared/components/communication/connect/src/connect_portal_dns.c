...
esp_err_t connect_internal_portal_dns_process_once(void)
{
    ...
    const int          len = recvfrom(sock, packet, sizeof(packet), 0, (struct sockaddr *) &source_addr, &source_len);
    if (len < 0)
    {
        taskENTER_CRITICAL(&s_dns_lock);
    ...
    if (sendto(sock, packet, (size_t) response_len, 0, (const struct sockaddr *) &source_addr, source_len) < 0)
    {
        return ESP_FAIL;
    }
    return ESP_OK;
