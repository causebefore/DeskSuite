 ...
 * @note
 * 除状态快照外，初始化、配置、启动、停止和反初始化接口必须由编排方串行调用
 */
#pragma once
 ...
 *
 * 初始化后启用已经通过链接器接入的 Log V2 捕获，立即开始非阻塞缓存普通 ESP_LOGx
 * 输出， 但不创建上传 Task。链接器包装继续调用
 * esp_log_va()，因此原串口输出保持不变。队列满时 拒绝最新日志并累计
 * dropped_lines。本组件不设置或独占 esp_log_set_vprintf()。
 *
 * @param[in] config 资源与重试配置，仅在调用期间借用并完整复制
 * @return ESP_OK 成功；ESP_ERR_INVALID_ARG 配置无效；ESP_ERR_INVALID_STATE
 * 已初始化； ESP_ERR_NO_MEM 资源创建失败
 */
    esp_err_t remote_log_init(const remote_log_config_t *config);

    /**
 * @brief 复制完整后端连接、鉴权与设备身份上下文
 *
 * 仅允许在上传 Task 停止时调用。修改配置会清空已有
 * session_id，但不会清空已缓存日志。 本组件直接使用
 * protocols/log_upload，不读取 Storage；日志启动与批次请求都会携带上下文 中的
 * Bearer Token 与稳定设备 ID。
 *
 ...
 * @brief 启动独立上传 Task
 *
 * 调用前必须完成 network_manager_init_borrow() 和
 * remote_log_configure_copy()。本函数不启动或停止 Network Manager；Task
 * 会读取其状态，仅在 NETWORK_STATE_ONLINE 时同步调用 log_upload。
 *
 ...
 * @brief 同步停止上传 Task
 *
 * 停止请求不会取消正在执行的同步 log_upload 调用；函数等待其返回并让 Task
 * 到达安全停止点。 尚未上传的当前内存批次会计入
 * dropped_lines，队列中未取出的日志保留供后续 start 使用。 超时后保持
 * REMOTE_LOG_STATE_STOPPING，可再次调用本函数继续等待。
 *
 ...
 *
 * 调用方必须保证上传 Task
 * 已停止。函数先停用捕获并等待已经进入捕获流程的调用退出，再释放
