...
邮箱页只展示 Dashboard 已按未读优先排序的前两封邮件：未读不足两封时由最近已读邮件补位，
没有未读时仍展示最近两封已读邮件。主题使用最多两行，未读以黑条和半粗黑字强调，已读使用
浅灰常规字重；页面顶部始终显示完整未读计数。
...
设置根菜单按“网络设置 → 网页文件管理 → 系统信息 → 检查更新”排列且焦点循环（末项按右键绕回首项，首项按左键绕回末项）。进入
...
- 本轮未运行测试、网页生成器、固件编译、截图或实机交互检查；后续编译需通过统一命令
