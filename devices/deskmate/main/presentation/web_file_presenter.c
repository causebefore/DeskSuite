...
static web_file_view_model_t s_view;
static bool                  s_initialized;
static uint64_t              s_applied_revision;

/** @brief 返回 Presenter 状态对应的固定中文标题 */
static const char *title_for_state(web_file_presenter_state_t state)
{
    switch (state)
    {
        case WEB_FILE_PRESENTER_STATE_STOPPED:
            return "网页文件管理";
        case WEB_FILE_PRESENTER_STATE_CHECKING_STORAGE:
            return "正在检查 SD 卡";
        case WEB_FILE_PRESENTER_STATE_ACQUIRING_NETWORK:
            return "正在申请网络";
        case WEB_FILE_PRESENTER_STATE_STARTING_SERVICE:
            return "正在启动服务";
        case WEB_FILE_PRESENTER_STATE_RUNNING:
            return "网页文件管理已开启";
        case WEB_FILE_PRESENTER_STATE_STOPPING:
            return "正在关闭服务";
        case WEB_FILE_PRESENTER_STATE_ERROR:
        default:
            return "启动或关闭失败";
    }
}
...
static web_file_view_model_t make_stopped_view(void)
{
    web_file_view_model_t view = {
        .state        = WEB_FILE_PRESENTER_STATE_STOPPED,
        .exit_allowed = true,
        .error        = ESP_OK,
    };
    (void) snprintf(view.title, sizeof(view.title), "%s", "网页文件管理");
...
{
    const web_file_view_model_t initial = make_stopped_view();
...
esp_err_t web_file_presenter_update_copy(const web_file_presenter_input_t *input, bool *out_accepted)
{
    if (input == NULL || out_accepted == NULL || input->presentation_revision == 0U
        || (int) input->state < (int) WEB_FILE_PRESENTER_STATE_STOPPED || input->state > WEB_FILE_PRESENTER_STATE_ERROR)
    {
    ...
    *out_accepted              = false;
    ...
    web_file_view_model_t view = {
        .state        = input->state,
        .running      = input->state == WEB_FILE_PRESENTER_STATE_RUNNING,
