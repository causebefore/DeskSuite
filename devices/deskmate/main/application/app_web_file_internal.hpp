 ...
 *
 * 本函数只从网页文件 Application Task 调用。链路与 Service 快照在产品状态锁外读取；
 ...
 * 不重复发布。只允许网页文件 Application Task 在无待处理停止请求时调用。
 *
...
/** @brief 记录或清除当前 Application 持有的 Web 文件租约代次 */
void app_web_file_internal_set_lease_generation(uint32_t generation);

/** @brief 读取当前 Application 持有的 Web 文件租约代次 */
...
/**
 * @brief 合并 Network Manager 链路变化事实并唤醒活动网页文件 Task
 *
