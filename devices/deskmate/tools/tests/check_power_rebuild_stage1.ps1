...
$networkKconfig = 'main\Kconfig.projbuild'
...
Assert-Contains $powerTask 'voice_service_get_status_copy'
...
Assert-Contains $powerTask 'app_network_get_next_dashboard_sync_interval_ms'
...
Assert-Contains $networkTask `
    '(?s)s_dashboard_failure_retry_multipliers\[\].*?\{\s*1U,\s*5U,\s*15U,\s*60U\s*\}'
Assert-Contains $networkTask 's_dashboard_failure_retry_stage\s*=\s*0U'
...
Assert-Contains $networkTask `
    '(?s)reconcile_portal_activity_deadline\s*\(.*?portal_activity_sequence.*?APP_NETWORK_PORTAL_ACTIVITY_WINDOW_SEC'
Assert-Contains $networkTask `
...
Assert-Contains $networkKconfig 'DESKMATE_LIGHT_SLEEP_REFRESH_INTERVAL_SEC'
Assert-Contains $networkKconfig `
    '(?s)DESKMATE_DASHBOARD_FAILURE_RETRY_SEC.*?1、5、15、60 倍间隔'
Assert-NotContains $networkKconfig 'DESKMATE_RTC_INT_WAKE_TEST_ENABLED'
...
Assert-NotContains $networkKconfig 'DESKMATE_POWER_VALIDATION_MODE'
Assert-NotContains 'sdkconfig.defaults' 'CONFIG_DESKMATE_POWER_VALIDATION_MODE'
Assert-NotContains $networkKconfig 'DESKMATE_LIGHT_SLEEP_PREPARE_TIMEOUT_MS'
