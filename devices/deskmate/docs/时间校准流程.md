...
- **SNTP 网络时间**：Wi-Fi 联网后的最高优先级校准来源；共享 Communication
  `time_sync` 只取得单次网络样本，DeskMate `system_clock` 校验并接受后更新 ESP32 系统
  时钟，再尽力回写 PCF85063。
...
flowchart TD
    ...
    Connected -->|"是"| GetSNTP["调用共享 time_sync<br/>请求第一个 SNTP 样本"]
...
2. `system_clock_sync_from_rtc()` 通过 `device_rtc_read_snapshot()` 读取完整快照。PCF85063
...
4. `network_manager` 发布 `NETWORK_STATE_ONLINE` 后，`app_network` Task 立即调用一次
   `system_clock_sync_from_sntp()`。`system_clock` 使用
   `CONFIG_DESKMATE_TIME_SNTP_PRIMARY_SERVER` 调用共享
   `time_sync_sample_sntp_once_copy()`，每个样本采用有界超时。`time_sync` 串行管理
   ESP-NETIF 全局 SNTP 客户端，但不决定候选是否可信。
...
10. GPIO15 的下降沿只在 ISR 中通知 `rtc_service`；Service Task 再读取并清除 AF，释放低电平
    INT 后向 Application 发布事件。中断处理不修改系统时钟，也不参与 SNTP 可信来源选择。
11. PCF85063 Driver 初始化时保留 AIE、AF、告警比较和 CLKOUT 配置，使启动前已经发生的有效
