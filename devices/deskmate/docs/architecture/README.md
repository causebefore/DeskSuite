...
- 冷启动优先启动 UI Runtime 并派发首屏，音频模型、AFE 和语音 Service 等耗时能力随后
  初始化。
- 在尚未提供“能力未就绪”页面状态前，`button_service` 必须等全部页面依赖就绪后再启动，
  避免按键进入未初始化或未启动的产品能力。
- 这项顺序只定义冷启动交互优先级，不改变各组件已有的 `init/start/stop/deinit` 生命周期
  契约，也不替代后续对完整失败回滚表的确认。

## 尚未确认的边界

以下内容不得由 AI 自行补全为规则，后续需要继续讨论：

1. 除上述已确认的冷启动交互优先级外，各组件精确的停止顺序、其余启动依赖，以及部分启动
   失败时的完整回滚表。
