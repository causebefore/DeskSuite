...
DeskMate 在 `app_main_start()` 完成 UI、RTC、电源与按键等本地关键运行时启动后确认当前镜像，
不把服务端或 Wi‑Fi 可用性作为健康门槛。
...
检查路径由 `CONFIG_FIRMWARE_OTA_CHECK_PATH` 注入。DeskMate 固定使用
`/api/v1/deskmate/ota/check`，不得在共享服务端场景回退到 PhotoPainter 使用的通用路径。
服务基础地址、设备令牌和设备 ID 由 `app_network` 在每次检查前复制，Tool 不读取产品设置。
