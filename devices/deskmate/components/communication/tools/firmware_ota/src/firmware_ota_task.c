...
static esp_err_t firmware_ota_parse_target(const char *body, firmware_ota_target_t *out_target, bool *out_has_update)
{
    *out_has_update = false;
    ...
    if (!cJSON_IsNumber(protocol_version) || protocol_version->valueint != 1 || !cJSON_IsObject(updates))
    {
                                           ...
                                           bool *out_has_update)
{
    ...
    ESP_RETURN_ON_ERROR(protocol_url_build(url, sizeof(url), s_runtime.base_url, CONFIG_FIRMWARE_OTA_CHECK_PATH),
                        TAG,
                        "构造 OTA 检查地址失败");
    char *body = NULL;
 ...
 */
esp_err_t firmware_ota_configure_copy(const char *base_url, const char *token, const char *device_id)
{
