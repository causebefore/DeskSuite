...
static_assert(std::is_trivially_copyable_v<transport_websocket_message_t>, "WebSocket 队列消息必须可按值复制");
...
struct transport_websocket
{
    ...
    esp_err_t open(const transport_websocket_config_t &config)
    {
        ...
        receive_queue     = xQueueCreate(config.receive_queue_length, sizeof(transport_websocket_message_t));
        ...
        esp_err_t err     = esp_websocket_register_events(client, WEBSOCKET_EVENT_ANY, handle_event, this);
        ...
        if (err == ESP_OK
            && (xSemaphoreTake(connection_signal, pdMS_TO_TICKS(config.connect_timeout_ms)) != pdTRUE
                || !connected.load()))
        {
            err = ESP_ERR_TIMEOUT;
        }
    ...
    void on_event(int32_t id, esp_websocket_event_data_t *event)
    {
        ...
        const bool binary =
            event->op_code == WS_TRANSPORT_OPCODES_BINARY || event->op_code == WS_TRANSPORT_OPCODES_CONT;
        if (event->payload_offset == 0)
        {
            ...
            assemble_data = static_cast<uint8_t *>(
                heap_caps_malloc(static_cast<size_t>(event->payload_len), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
        ...
        if (assemble_data == nullptr || assemble_offset + static_cast<size_t>(event->data_len) > assemble_len)
        {
            return;
        }

        memcpy(assemble_data + assemble_offset, event->data_ptr, static_cast<size_t>(event->data_len));
    ...
    std::atomic_bool              connected         = false;   /**< 当前是否已连接 */
    std::atomic_bool              disconnected      = false;   /**< 是否收到断开或错误事件 */
    ...
    std::atomic_uint32_t          dropped_messages  = 0;       /**< 丢弃消息累计数 */
...
{
    if (config == nullptr || config->url == nullptr || out == nullptr || config->receive_queue_length == 0
...
esp_err_t transport_websocket_send_text(transport_websocket_t *socket, const char *text, uint32_t timeout_ms)
{
    ...
    return checked_send(
        esp_websocket_client_send_text(socket->client, text, static_cast<int>(len), pdMS_TO_TICKS(timeout_ms)),
        len);
}
                                      ...
                                      uint32_t timeout_ms)
{
    ...
    return socket->disconnected.load() ? ESP_ERR_INVALID_STATE : ESP_ERR_TIMEOUT;
}
...
{
    return socket != nullptr ? socket->dropped_messages.load() : 0;
}
