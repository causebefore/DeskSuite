...
#define LOG_UPLOAD_SESSION_ID_MAX 96
    ...
    typedef struct
    {
        uint32_t    product_id;       /**< 大于 0 的产品标识 */
        const char *device_id;        /**< 非空设备 ID */
     ...
     * 缓冲区；参数校验通过后函数会先清空该缓冲区，仅返回 ESP_OK 时其中的会话 ID 有效。
     *
     ...
     *
     * 函数仅在调用期间借用所有输入指针。session_id 可为空；next_session_id 可为 NULL。
     * 服务端未返回新会话 ID 时不会改写 next_session_id，调用方需要区分该情况时应预先清空。
     *
     * @param[in] base_url 服务端基础地址
     * @param[in] timeout_ms HTTP 请求超时，单位毫秒
    ...
    esp_err_t log_upload_batch(const char *base_url, int timeout_ms, uint32_t product_id, const char *device_id,
                               const char *session_id, const log_upload_line_t *lines, size_t count,
                               char *next_session_id, size_t next_session_id_len);
