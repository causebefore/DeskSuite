 ...
 */
static esp_err_t display_frame_protocol_check_sha256(const uint8_t *data, size_t size_bytes, const char *expected_hex)
{
                                                 ...
                                                 display_frame_protocol_view_t *out_view)
{
    ...
    if (frame_size != DISPLAY_FRAME_PROTOCOL_FILE_SIZE || expected_page->file_size != DISPLAY_FRAME_PROTOCOL_FILE_SIZE)
    {
        return ESP_ERR_INVALID_SIZE;
    }

    uint64_t expected_version;
    if (memcmp(frame_data, DISPLAY_FRAME_PROTOCOL_FORMAT, 4U) != 0 || frame_data[4] != DISPLAY_FRAME_PROTOCOL_VERSION
        ...
        || utils_read_le64(frame_data + 20U) != expected_version || utils_read_le32(frame_data + 28U) != 0U)
    {
    ...
    esp_err_t error = display_frame_protocol_check_sha256(payload,
                                                          DISPLAY_FRAME_PROTOCOL_PAYLOAD_SIZE,
                                                          expected_page->payload_sha256);
    if (error == ESP_OK)
    {
