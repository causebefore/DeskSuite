...
esp_err_t protocol_url_build(char *out, size_t out_len, const char *base_url, const char *path)
{
    ...
    const size_t scheme_length = has_http ? strlen("http://") : (has_https ? strlen("https://") : 0U);
    ...
    const int   written =
        snprintf(out, out_len, "%s%.*s%s%s", default_scheme, (int) base_length, base_start, separator, path_start);
