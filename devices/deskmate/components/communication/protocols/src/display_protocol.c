...
{
    return url != NULL && url[0] == '/' && strlen(url) < DISPLAY_PROTOCOL_URL_MAX && strstr(url, "://") == NULL
           && strstr(url, "..") == NULL;
}
...
{
    ESP_RETURN_ON_FALSE(cJSON_IsObject(item) && out_page != NULL, ESP_ERR_INVALID_RESPONSE, TAG, "显示页面对象无效");
    ...
    const bool valid = cJSON_IsString(page_id) && display_protocol_page_id_is_valid(page_id->valuestring)
                       ...
                       && cJSON_IsString(crc32) && utils_is_hex_string(crc32->valuestring, 8U) && cJSON_IsString(sha256)
                       && utils_is_hex_string(sha256->valuestring, 64U) && cJSON_IsString(payload_sha256)
                       && utils_is_hex_string(payload_sha256->valuestring, 64U) && cJSON_IsString(frame_url)
    ...
    snprintf(out_page->content_version, sizeof(out_page->content_version), "%s", version->valuestring);
    ...
    snprintf(out_page->payload_sha256, sizeof(out_page->payload_sha256), "%s", payload_sha256->valuestring);
...
static esp_err_t display_protocol_parse_manifest(const char *body, display_protocol_manifest_t *out)
{
    ...
    bool valid = cJSON_IsNumber(protocol) && protocol->valueint == 3 && cJSON_IsString(format)
                 && strcmp(format->valuestring, DISPLAY_FRAME_PROTOCOL_FORMAT) == 0 && cJSON_IsString(pixel_format)
                 && strcmp(pixel_format->valuestring, DISPLAY_FRAME_PROTOCOL_PIXEL_FORMAT) == 0
                 && cJSON_IsString(version) && display_protocol_version_is_valid(version->valuestring)
                 && cJSON_IsString(default_page) && display_protocol_page_id_is_valid(default_page->valuestring)
                 && cJSON_IsNumber(width) && width->valueint == (int) DISPLAY_FRAME_PROTOCOL_WIDTH
                 && cJSON_IsNumber(height) && height->valueint == (int) DISPLAY_FRAME_PROTOCOL_HEIGHT
                 && cJSON_IsNumber(bpp) && bpp->valueint == (int) DISPLAY_FRAME_PROTOCOL_BITS_PER_PIXEL
                 && cJSON_IsNumber(header_size) && header_size->valueint == (int) DISPLAY_FRAME_PROTOCOL_HEADER_SIZE
                 && cJSON_IsNumber(payload_size)
                 && payload_size->valuedouble == (double) DISPLAY_FRAME_PROTOCOL_PAYLOAD_SIZE
                 && cJSON_IsNumber(file_size) && file_size->valuedouble == (double) DISPLAY_FRAME_PROTOCOL_FILE_SIZE
    ...
    for (int index = 0; index < page_count_json; ++index)
    {
        ...
        const esp_err_t page_err = display_protocol_parse_page(item, out->file_size, &out->pages[index]);
        if (page_err != ESP_OK)
        {
            cJSON_Delete(root);
            return page_err;
 ...
 */
esp_err_t display_protocol_get_manifest_copy(const char *in_base_url, const char *in_token, const char *in_device_id,
                                             const char *in_current_version, int64_t in_current_next_refresh_at_utc,
                                             int timeout_ms, display_protocol_manifest_t *out_manifest)
{
    ESP_RETURN_ON_FALSE(in_base_url != NULL && in_base_url[0] != '\0' && in_device_id != NULL && in_device_id[0] != '\0'
                            && out_manifest != NULL,
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "显示 Manifest 参数无效");
    ...
    char url[256];
    ESP_RETURN_ON_ERROR(protocol_url_build(url, sizeof(url), in_base_url, "/api/v2/display/manifest"),
                        TAG,
    ...
    char                    bearer[128]  = { 0 };
    char                    etag[64]     = { 0 };
    protocol_identity_add_headers(headers, &header_count, in_token, in_device_id, bearer, sizeof(bearer));
    headers[header_count++] = (transport_http_header_t) {
        .name  = "Accept",
        .value = "application/json",
    };
    if (in_current_version != NULL && in_current_version[0] != '\0' && in_current_next_refresh_at_utc > 0)
    {
        snprintf(etag, sizeof(etag), "\"%s:%lld\"", in_current_version, (long long) in_current_next_refresh_at_utc);
        headers[header_count++] = (transport_http_header_t) {
            .name  = "If-None-Match",
            .value = etag,
        };
    ...
    ESP_LOGI(TAG,
             ...
             "timeout=%d ms",
             in_current_version != NULL && in_current_version[0] != '\0' ? in_current_version : "无",
...
esp_err_t display_protocol_download_frame_borrow(const char *in_base_url, const char *in_token,
                                                 const char *in_device_id, const char *in_frame_url, int timeout_ms,
                                                 transport_http_data_cb_t in_callback, void *in_context,
                                                 transport_http_download_result_t *out_result)
{
    ESP_RETURN_ON_FALSE(in_base_url != NULL && in_base_url[0] != '\0' && in_device_id != NULL && in_frame_url != NULL
                        ...
                        "显示帧下载参数无效");
    char url[320];
    ESP_RETURN_ON_ERROR(protocol_url_build(url, sizeof(url), in_base_url, in_frame_url), TAG, "构造显示帧 URL 失败");
