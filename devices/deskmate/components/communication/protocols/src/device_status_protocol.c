...
static bool device_status_protocol_values_are_valid(const device_status_protocol_upload_t *status)
{
    ...
    return !status->has_environment
           || (isfinite(status->temperature_c) && status->temperature_c >= -50.0F && status->temperature_c <= 100.0F
               && isfinite(status->humidity_percent) && status->humidity_percent >= 0.0F
               && status->humidity_percent <= 100.0F);
}

esp_err_t device_status_protocol_upload_borrow(const char *in_base_url, const char *in_token, const char *in_device_id,
                                               const device_status_protocol_upload_t *in_status, int timeout_ms)
{
    ...
    cJSON *environment = in_status->has_environment ? cJSON_AddObjectToObject(root, "environment") : NULL;
    cJSON *battery     = cJSON_AddObjectToObject(root, "battery");
    bool   complete = battery != NULL && cJSON_AddNumberToObject(battery, "percent", in_status->battery_percent) != NULL
                      && cJSON_AddNumberToObject(battery, "voltage_mv", in_status->battery_voltage_mv) != NULL;
    if (in_status->has_environment)
    {
        complete = complete && environment != NULL
                   && cJSON_AddNumberToObject(environment, "temperature_c", in_status->temperature_c) != NULL
                   && cJSON_AddNumberToObject(environment, "humidity_percent", in_status->humidity_percent) != NULL;
    }
