...
- 自动化测试：本任务按约束未修改或执行测试。
- 静态核查：检查受控 SDK 配置中的 UTF-8、LFN、HTTPD WebSocket 关闭与 2048 字节 URI
...
1. 进入设置子页，观察存储检查、网络租约和 Service 启动阶段。
2. 核对 URL 使用当前 STA IPv4，六位访问码能够保留前导零。
3. 一个浏览器登录后，第二个全新浏览器应被拒绝。
4. 浏览根目录、普通隐藏项、中文目录、空格、`%`、`#` 和多字节边界名称。
5. 确认 `.deskmate-web` 不出现在列表，直接编码访问返回 `400` 或 `404`。
6. 下载文本、图片和二进制样本，比较长度与哈希。
7. 上传 0 字节、32 KiB、非块对齐和 500 MiB 文件，比较哈希。
8. 确认 500 MiB + 1 返回 `413`、空间不足返回 `507`、未确认覆盖返回 `409`。
9. 确认覆盖被批准后，旧文件在新文件 durable 前保持完整。
10. 在每个 journal 阶段中断网络或电源，验证恢复结果确定且可重复。
11. 检查上传进度、UI 响应、单传输忙响应和十分钟会话空闲过期。
12. 检查上传取消后 `.part` 被清理，失败文件可以重试且仍保持单文件上传。
13. 检查容量显示、名称筛选、五种排序方式和筛选结果全选状态。
14. 创建中文目录，重命名和移动常规文件，确认目标冲突不会覆盖既有文件。
15. 顺序移动或删除多个文件，确认失败项逐项报告；非空目录、根目录和保留目录不能删除。
16. 确认活动租约阻止 Light-sleep、Portal、语音租约、OTA 和 Dashboard 同步。
17. 离开设备子页，确认 HTTPD Task、socket、文件句柄、PSRAM、秘密和租约按顺序释放。
18. Service 仍启用时重连 STA，确认 URL 更新且访问码不变。

500 MiB 上传期间还需记录普通堆和 PSRAM 的当前空闲量与历史最低空闲量。
