...
负责把内容刷新、显示收敛、180 秒操作窗口、固件检查/确认/退出和深睡串行化到同一个 Task。
...
- 成功轮次使用绝对 UTC 计划；失败跨深睡保留 1/5/15 分钟退避。
- 自动休眠前必须确认网络已经关闭、照片播放不在刷新、目标集合已显示或明确无内容。
- 网络、显示或恢复错误进入 `AUTO_SLEEP_BLOCKED`；该状态拒绝手动和自动深睡。
...
- 任一网络清理或照片恢复失败进入 `AUTO_SLEEP_BLOCKED`。
...
SD 和网络；准备失败则反向恢复。OTA 手动退出后内容 Task 可保持停止到休眠，确认键长按会重新
启动它；若休眠准备失败，本 App 也会恢复该 Task，避免停留在无事件来源状态。

运行期尚未初始化时，`bootstart_app` 可以调用同步启动失败深睡入口。该入口复用相同停机顺序，
但把尚未初始化的照片、内容、OTA、日志、SD、网络和显示视为无需清理，以便系统基础或局部
初始化失败也能进入 1/5/15 分钟退避深睡。这个宽松规则只用于启动失败收敛；运行期自动和手动
休眠仍要求网络、显示及所有已启动资源达到严格终态。
