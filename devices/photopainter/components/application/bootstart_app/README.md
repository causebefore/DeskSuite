...
- 在配网前初始化共用按键 Device/Service；配网 App 退出后再由照片播放注册回调并启动扫描。
- 维护所有非照片状态画面的一次性启动恢复适配，在首次正常页面物理刷新成功后清除标记。
...
SD → 关键显示/网络/日志/OTA → 按键输入 → 配网并启动日志上传 → 照片与内容链路 → 反馈设备 → 电源管理 →
首轮内容刷新。
...
- 服务地址为空由启动配网用例转为 `NO SERVER` 提示，不再作为照片链路致命启动错误；其他
...
- 非照片状态画面恢复标记由配网或运行期电源管理流程设置；启动编排只读取该事实，并在正常页面物理
