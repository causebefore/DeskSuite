 ...
 */
static esp_err_t system_clock_datetime_to_utc(const device_rtc_datetime_t *datetime,
                                              int16_t utc_offset_minutes,
                                              time_t *out_timestamp)
{
    ESP_RETURN_ON_FALSE(datetime != NULL && out_timestamp != NULL,
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "RTC 时间转换参数无效");
    const uint8_t month_days = system_clock_days_in_month(datetime->year, datetime->month);
    ESP_RETURN_ON_FALSE(datetime->year >= 2000U && datetime->year <= 2099U
                            && month_days > 0U && datetime->day >= 1U
                            && datetime->day <= month_days && datetime->hour <= 23U
    ...
    const uint32_t day_of_year =
        (153U * shifted_month + 2U) / 5U + (uint32_t) datetime->day - 1U;
    const uint32_t day_of_era =
        year_of_era * 365U + year_of_era / 4U - year_of_era / 100U + day_of_year;
    const int64_t days_since_epoch =
        (int64_t) era * 146097LL + (int64_t) day_of_era - 719468LL;
    const int64_t local_seconds =
        days_since_epoch * 86400LL + (int64_t) datetime->hour * 3600LL
        + (int64_t) datetime->minute * 60LL + (int64_t) datetime->second;
    const time_t utc_timestamp =
        (time_t) (local_seconds - (int64_t) utc_offset_minutes * 60LL);
 ...
 */
static esp_err_t system_clock_utc_to_datetime(time_t timestamp,
                                              int16_t utc_offset_minutes,
                                              device_rtc_datetime_t *out_datetime)
{
    ESP_RETURN_ON_FALSE(out_datetime != NULL,
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "RTC 回写时间转换参数无效");
    const time_t local_timestamp =
        timestamp + (time_t) ((int64_t) utc_offset_minutes * 60LL);
    ...
    ESP_RETURN_ON_FALSE(year >= 2000 && year <= 2099,
                        ESP_ERR_INVALID_RESPONSE,
                        TAG,
                        "RTC 回写年份超出支持范围");
...
static esp_err_t system_clock_get_trusted_time(time_t *timestamp, system_clock_source_t *source)
{
    ...
    *timestamp = anchor_timestamp
                 + (time_t) ((now_monotonic_us - anchor_monotonic_us) / SYSTEM_CLOCK_US_PER_SECOND);
    if (source != NULL)
    {
...
static void system_clock_write_sntp_time_to_rtc(time_t timestamp)
{
    ...
    esp_err_t error =
        system_clock_utc_to_datetime(timestamp, utc_offset_minutes, &datetime);
    if (error == ESP_OK)
    {
    ...
    {
        ESP_LOGW(TAG,
                 "SNTP 时间已接受，但回写设备 RTC 失败，系统时间继续有效：%s",
...
static void system_clock_load_utc_offset(void)
{
    ...
    if (err == ESP_OK)
    {
        ...
        const esp_err_t persist_err =
            system_storage_set_utc_offset_minutes(SYSTEM_CLOCK_DEFAULT_UTC_OFFSET_MINUTES);
...
esp_err_t system_clock_get_snapshot_copy(system_clock_snapshot_t *out_snapshot)
{
    ...
    const esp_err_t trusted_err =
        system_clock_get_trusted_time(&snapshot.utc_timestamp, &snapshot.source);
...
esp_err_t system_clock_set_time(time_t timestamp)
{
    ...
    const bool initialized   = s_initialized;
    taskEXIT_CRITICAL(&s_state_lock);
    ESP_RETURN_ON_FALSE(initialized && s_writer_mutex != NULL,
                        ESP_ERR_INVALID_STATE,
                        TAG,
                        "系统时钟未初始化");
    ESP_RETURN_ON_FALSE(system_clock_timestamp_is_valid(timestamp),
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "SNTP 时间超出可信范围");
    ...
    {
        const int64_t jump_seconds =
            system_clock_timestamp_difference(timestamp, trusted_timestamp);
        ...
        const bool confirmation_required =
            ...
            || (!s_confirmation_pending
                && jump_seconds > SYSTEM_CLOCK_MAX_UNCONFIRMED_JUMP_SECONDS);
        if (confirmation_required)
        {
            ...
            ESP_LOGW(TAG,
                     "SNTP 候选与可信时间相差 %lld 秒，等待第二个相近样本确认",
                     (long long) jump_seconds);
...
esp_err_t system_clock_sync_from_sntp(const char *server, uint32_t timeout_ms)
{
    ...
    ESP_LOGI(TAG,
             "开始 SNTP 网络校时：server=%s，单样本超时=%lu ms",
             server,
    ...
    if (result != ESP_OK)
    {
        const uint64_t total_ms =
            (uint64_t) (esp_timer_get_time() - started_us + 999LL) / 1000ULL;
        ESP_LOGE(TAG,
    ...
    while (sample_count < 2U)
    {
        ...
        had_trusted_at_sample =
            system_clock_get_trusted_time(&trusted_at_sample, NULL) == ESP_OK;
    ...
    const int64_t correction_seconds =
        had_trusted_at_sample ? (int64_t) candidate - (int64_t) trusted_at_sample : 0LL;
    ESP_LOGI(TAG,
...
esp_err_t system_clock_sync_from_rtc(void)
{
    ...
    time_t timestamp;
    result = system_clock_datetime_to_utc(
        &snapshot.datetime, utc_offset_minutes, &timestamp);
    if (result != ESP_OK)
    {
        goto done;
    }
