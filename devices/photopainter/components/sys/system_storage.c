...
{
    return config != NULL && config->version == SYSTEM_STORAGE_NETWORK_CONFIG_VERSION
           && config->ssid[0] != '\0'
           ...
           && system_storage_string_is_terminated(config->device_token,
...
esp_err_t system_storage_set_network_config_borrow(const system_storage_network_config_t *config)
{
    ...
    esp_err_t err =
        nvs_set_blob(handle, SYSTEM_STORAGE_KEY_NETWORK_CONFIG, config, sizeof(*config));
    if (err == ESP_OK)
    {
...
{
    ESP_RETURN_ON_FALSE(system_storage_daily_alarm_is_valid(alarm),
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "每日告警配置无效");
