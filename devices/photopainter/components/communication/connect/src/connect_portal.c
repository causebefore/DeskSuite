...
_Static_assert(sizeof(CONFIG_CONNECT_PORTAL_SSID) > 1U
                   && sizeof(CONFIG_CONNECT_PORTAL_SSID) <= CONNECT_WIFI_SSID_MAX,
               "配网热点 SSID 长度必须为 1 至 32 字节");
_Static_assert(sizeof(CONFIG_CONNECT_PORTAL_PASSWORD) >= 9U
                   && sizeof(CONFIG_CONNECT_PORTAL_PASSWORD) <= 64U,
...
static esp_err_t save_post_handler(httpd_req_t *req)
{
    ...
    {
        const int received =
            httpd_req_recv(req, body + received_total, req->content_len - received_total);
