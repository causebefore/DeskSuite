...
esp_err_t connect_internal_portal_scan_stop(void)
{
    ...
    if (scan_in_progress)
    {
        ...
        if (scan_err != ESP_OK && scan_err != ESP_ERR_WIFI_NOT_STARTED
            && scan_err != ESP_ERR_WIFI_NOT_INIT && scan_err != ESP_ERR_WIFI_STATE)
        {
            ESP_LOGW(TAG, "请求停止 Wi-Fi 扫描失败：%s", esp_err_to_name(scan_err));
        }
    }

    if (xSemaphoreTake(s_scan_stopped, pdMS_TO_TICKS(CONNECT_PORTAL_SCAN_STOP_TIMEOUT_MS))
        != pdTRUE)
    {
