 ...
 */
static void remote_log_capture(esp_log_config_t config, const char *tag, const char *format,
                               va_list args)
{
    taskENTER_CRITICAL(&g_remote_log_lock);
    const bool capture_enabled =
        g_remote_log_runtime.initialized && g_remote_log_runtime.capture_enabled;
    ...
    if (!capture_enabled || events == NULL || log_slots == NULL
        || xTaskGetCurrentTaskHandle() == uploader_task)
    {
        if (capture_enabled)
        {
