...
{
    return config != NULL && config->queue_capacity > 0U
           && config->queue_capacity <= REMOTE_LOG_QUEUE_CAPACITY_MAX && config->batch_capacity > 0U
           && config->batch_capacity <= config->queue_capacity && config->retry_interval_ms > 0U
           && config->http_timeout_ms > 0 && config->task_stack_size_bytes > 0U
...
{
    taskENTER_CRITICAL(&g_remote_log_lock);
    g_remote_log_runtime.state =
        g_remote_log_runtime.stop_requested ? REMOTE_LOG_STATE_STOPPING : state;
...
static void remote_log_wait_retry(void)
{
    (void) ulTaskNotifyTake(pdTRUE,
                            remote_log_ms_to_ticks(g_remote_log_runtime.config.retry_interval_ms));
}
...
static void remote_log_discard_pending_stop(void)
{
    ...
    if (xQueuePeek(g_remote_log_runtime.events, &event, 0U) == pdTRUE
        && event.type == REMOTE_LOG_EVENT_STOP)
    {
...
static bool remote_log_collect_batch(size_t *out_count)
{
    *out_count = 0U;
    ...
    {
        if (!remote_log_receive_event(
                &event,
                remote_log_ms_to_ticks(g_remote_log_runtime.config.batch_wait_ms)))
        {
            break;
        }
...
esp_err_t remote_log_init(const remote_log_config_t *config)
{
    ...
    QueueHandle_t events =
        xQueueCreate((UBaseType_t) (config->queue_capacity + 1U), sizeof(remote_log_event_t));
    if (events == NULL)
    {
        free(batch);
        return ESP_ERR_NO_MEM;
    }
    SemaphoreHandle_t log_slots = xSemaphoreCreateCounting((UBaseType_t) config->queue_capacity,
                                                           (UBaseType_t) config->queue_capacity);
    if (log_slots == NULL)
    {
        vQueueDelete(events);
        free(batch);
...
{
    if (!remote_log_string_fits(base_url, REMOTE_LOG_BASE_URL_MAX)
        || product_id == 0U || !remote_log_string_fits(device_id, REMOTE_LOG_DEVICE_ID_MAX))
    {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t remote_log_stop(uint32_t timeout_ms)
{
    ...
    const bool        already_stopping = g_remote_log_runtime.initialized
                                         ...
                                         && g_remote_log_runtime.task != NULL;
    const bool        can_request      = g_remote_log_runtime.initialized
                                         && g_remote_log_runtime.state != REMOTE_LOG_STATE_STOPPED
                                         && g_remote_log_runtime.task != NULL;
...
{
    taskENTER_CRITICAL(&g_remote_log_lock);
    const bool can_deinit = g_remote_log_runtime.initialized
                            && g_remote_log_runtime.state == REMOTE_LOG_STATE_STOPPED
                            && g_remote_log_runtime.task == NULL;
    if (can_deinit)
    {
...
esp_err_t remote_log_get_status_copy(remote_log_status_t *out_status)
{
    ...
    if (initialized)
    {
        ...
        memcpy(out_status->session_id,
               g_remote_log_runtime.session_id,
               sizeof(out_status->session_id));
    }
    taskEXIT_CRITICAL(&g_remote_log_lock);
