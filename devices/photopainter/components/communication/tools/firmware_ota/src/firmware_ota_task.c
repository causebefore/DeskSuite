                                                 ...
                                                 char out[FIRMWARE_OTA_ARTIFACT_ID_SIZE])
{
    ...
    ESP_RETURN_ON_ERROR(esp_partition_get_sha256(partition, digest),
                        TAG,
                        "读取应用镜像 Validation SHA-256 失败");
...
esp_err_t firmware_ota_get_identity_copy(firmware_ota_identity_t *out_identity)
{
    ...
    utils_copy_string(out_identity->current_version,
                      sizeof(out_identity->current_version),
                      description->version);
    out_identity->current_ota_version = FIRMWARE_OTA_BUILD_VERSION;
    ESP_RETURN_ON_ERROR(
        firmware_ota_partition_identity(esp_ota_get_running_partition(),
                                        out_identity->current_artifact_id),
        TAG,
        "读取当前固件身份失败");
    const esp_partition_t *last_invalid = esp_ota_get_last_invalid_partition();
    if (last_invalid != NULL)
    {
        const esp_err_t error = firmware_ota_partition_identity(
            last_invalid, out_identity->last_invalid_artifact_id);
        if (error == ESP_OK)
        {
 ...
 */
static esp_err_t firmware_ota_make_request_body(const firmware_ota_identity_t *identity,
                                                char **out_body)
{
    ...
    {
        cJSON_AddStringToObject(app,
                               "last_invalid_artifact_id",
                               identity->last_invalid_artifact_id);
    }
    else
    {
 ...
 */
static esp_err_t firmware_ota_copy_json_string(const cJSON *object,
                                               const char *name,
                                               char *out,
                                               size_t capacity)
{
 ...
 *
 * 校验规则：protocol_version 必须为 1、updates 必须为对象；app 子对象存在时，
 ...
 */
static esp_err_t firmware_ota_parse_target(const char *body,
                                           firmware_ota_target_t *out_target,
                                           bool *out_has_update)
{
    *out_has_update = false;
    ...
    const cJSON *app = cJSON_IsObject(updates)
                           ? cJSON_GetObjectItemCaseSensitive(updates, "app")
                           : NULL;
    esp_err_t error = ESP_OK;
    if (!cJSON_IsNumber(protocol_version) || protocol_version->valueint != 1
        || !cJSON_IsObject(updates))
    {
    ...
    else
    {
        ...
        const double ota_version_value = cJSON_IsNumber(ota_version)
                                             ? ota_version->valuedouble
                                             : 0.0;
        if (!cJSON_IsNumber(size) || size->valuedouble <= 0
            ...
            || firmware_ota_copy_json_string(app,
                                             "version",
                                             out_target->version,
                                             sizeof(out_target->version)) != ESP_OK
            || firmware_ota_copy_json_string(app,
                                             "artifact_id",
                                             out_target->artifact_id,
                                             sizeof(out_target->artifact_id)) != ESP_OK
            || firmware_ota_copy_json_string(app,
                                             "file_sha256",
                                             out_target->file_sha256,
                                             sizeof(out_target->file_sha256)) != ESP_OK
            || firmware_ota_copy_json_string(app,
                                             "url",
                                             out_target->url,
                                             sizeof(out_target->url)) != ESP_OK
            || !firmware_ota_is_sha256(out_target->artifact_id)
            || !firmware_ota_is_sha256(out_target->file_sha256)
            || out_target->url[0] != '/')
        {
            error = ESP_ERR_INVALID_RESPONSE;
        }
 ...
 * @return 实际写入的头数量
 */
static size_t firmware_ota_make_headers(transport_http_header_t headers[2],
                                        char bearer[FIRMWARE_OTA_TOKEN_MAX + 8U],
 ...
 */
static esp_err_t firmware_ota_check_target(const firmware_ota_identity_t *identity,
                                           firmware_ota_target_t *out_target,
                                           bool *out_has_update)
{
    char url[FIRMWARE_OTA_URL_MAX];
    ESP_RETURN_ON_ERROR(protocol_url_build(url,
                                           sizeof(url),
                                           s_runtime.base_url,
                                           "/api/v1/ota/check"),
                        TAG,
                        "构造 OTA 检查地址失败");
    char *body = NULL;
    ESP_RETURN_ON_ERROR(firmware_ota_make_request_body(identity, &body),
                        TAG,
                        "创建 OTA 检查请求失败");
...
static esp_err_t firmware_ota_on_download_data(const uint8_t *data, size_t length, void *context)
{
    ...
    ESP_RETURN_ON_FALSE(download != NULL && data != NULL,
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "OTA 下载回调参数无效");
    ESP_RETURN_ON_FALSE(download->received_bytes + length <= download->expected_size,
                        ...
                        TAG,
                        "OTA 下载数据超过清单大小");
    ESP_RETURN_ON_ERROR(esp_ota_write(download->ota_handle, data, length),
                        TAG,
                        "写入备用 OTA 分区失败");
    ESP_RETURN_ON_FALSE(mbedtls_md_update(&download->digest, data, length) == 0,
                        ESP_FAIL,
                        TAG,
                        "更新固件文件摘要失败");
...
static esp_err_t firmware_ota_download_and_activate(const firmware_ota_target_t *target)
{
    ...
    ESP_RETURN_ON_FALSE(target->size <= partition->size,
                        ESP_ERR_INVALID_SIZE,
                        TAG,
                        "目标固件超过备用 OTA 分区容量");
    ...
    esp_err_t error = digest_info != NULL
                          && mbedtls_md_setup(&download.digest, digest_info, 0) == 0
    ...
    if (error == ESP_OK && result.content_length >= 0
        && (size_t) result.content_length != target->size)
    {
        error = ESP_ERR_INVALID_SIZE;
    }
 ...
 *         其他值表示身份读取、网络或响应错误
 */
static esp_err_t firmware_ota_run_check(firmware_ota_check_result_t *out_result,
                                        firmware_ota_target_t *out_target)
{
    ...
    ESP_RETURN_ON_ERROR(firmware_ota_check_target(&identity, &target, &has_update),
                        TAG,
                        "检查 OTA 目标失败");
    if (!has_update)
    {
    ...
    if (strcmp(target.artifact_id, identity.current_artifact_id) == 0
        || (identity.has_last_invalid_artifact
            && strcmp(target.artifact_id, identity.last_invalid_artifact_id) == 0))
    {
    ...
    utils_copy_string(out_result->target_version,
                      sizeof(out_result->target_version),
                      target.version);
    utils_copy_string(out_result->target_artifact_id,
                      sizeof(out_result->target_artifact_id),
                      target.artifact_id);
    *out_target = target;
    return ESP_OK;
}
...
{
    ESP_RETURN_ON_FALSE(config != NULL && config->check_timeout_ms > 0
                            && config->download_timeout_ms > 0,
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "OTA 生命周期配置无效");
    ESP_RETURN_ON_FALSE(!s_runtime.initialized,
    ...
    {
        if (s_runtime.commands != NULL) vQueueDelete(s_runtime.commands);
        if (s_runtime.mutex != NULL) vSemaphoreDelete(s_runtime.mutex);
        if (s_runtime.task_stopped != NULL) vSemaphoreDelete(s_runtime.task_stopped);
        memset(&s_runtime, 0, sizeof(s_runtime));
        return ESP_ERR_NO_MEM;
    }
                                      ...
                                      const char *device_id)
{
    ESP_RETURN_ON_FALSE(firmware_ota_string_fits(base_url, FIRMWARE_OTA_BASE_URL_MAX, false)
                            && firmware_ota_string_fits(token != NULL ? token : "",
                                                        FIRMWARE_OTA_TOKEN_MAX,
                                                        true)
                            && firmware_ota_string_fits(device_id,
                                                        FIRMWARE_OTA_DEVICE_ID_MAX,
                                                        false),
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "OTA 服务连接配置无效");
    ESP_RETURN_ON_FALSE(s_runtime.initialized, ESP_ERR_INVALID_STATE, TAG, "OTA 工具尚未初始化");
    firmware_ota_lock();
    const bool allowed = !s_runtime.stopping
                         && (s_runtime.state == FIRMWARE_OTA_STATE_STOPPED
                             || s_runtime.state == FIRMWARE_OTA_STATE_IDLE);
    if (allowed)
    {
...
esp_err_t firmware_ota_start(void)
{
    ESP_RETURN_ON_FALSE(s_runtime.initialized, ESP_ERR_INVALID_STATE, TAG, "OTA 工具尚未初始化");
    firmware_ota_lock();
    const bool allowed = !s_runtime.started && !s_runtime.stopping
                         && s_runtime.state == FIRMWARE_OTA_STATE_STOPPED;
    if (allowed)
    {
...
esp_err_t firmware_ota_stop(void)
{
    ...
    const uint64_t wait_ms = (uint64_t) s_runtime.check_timeout_ms
                             + (uint64_t) s_runtime.download_timeout_ms
                             + FIRMWARE_OTA_STOP_GRACE_MS;
    return xSemaphoreTake(s_runtime.task_stopped, pdMS_TO_TICKS(wait_ms)) == pdTRUE
               ? ESP_OK
               : ESP_ERR_TIMEOUT;
}
...
esp_err_t firmware_ota_confirm_running_image(void)
{
    ...
    ESP_RETURN_ON_ERROR(esp_ota_mark_app_valid_cancel_rollback(),
                        TAG,
                        "确认本地健康启动失败");
