...
voice_protocol_decoder_t *voice_protocol_decoder_create(void)
{
    voice_protocol_decoder_t *decoder =
        heap_caps_calloc(1, sizeof(*decoder), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    return decoder;
}
...
esp_err_t voice_protocol_decoder_feed(voice_protocol_decoder_t *decoder,
                                      const uint8_t *data,
                                      size_t len,
                                      voice_protocol_frame_cb_t callback,
                                      void *ctx)
{
    ...
    while (pos < len)
    {
        if (decoder->header_pos < VOICE_PROTOCOL_HEADER_SIZE)
        {
            ...
            if (decoder->header_pos == VOICE_PROTOCOL_HEADER_SIZE)
            {
                decoder->payload_len = ((uint32_t) decoder->header[1] << 24)
                                       | ((uint32_t) decoder->header[2] << 16)
                                       | ((uint32_t) decoder->header[3] << 8)
                                       | (uint32_t) decoder->header[4];
                ...
                {
                    callback((voice_protocol_frame_type_t) decoder->header[0],
                             decoder->payload,
                             0,
                             ctx);
                    decoder_reset(decoder);
