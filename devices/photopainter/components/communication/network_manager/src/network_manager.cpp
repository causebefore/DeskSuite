...
esp_err_t network_manager_init_borrow(const network_manager_config_store_t *config_store)
{
    ...
    ESP_RETURN_ON_ERROR(network_manager_internal_task_init_borrow(config_store),
                        TAG,
                        "初始化网络管理任务失败");
...
{
    ESP_RETURN_ON_FALSE(network_manager_is_initialized(),
...
esp_err_t network_manager_get_portal_info_copy(connect_portal_info_t *out_info)
{
    ...
    bool available;
    taskENTER_CRITICAL(&s_public_state.lock);
    available = (s_public_state.status.state == NETWORK_STATE_PROVISIONING
    ...
    ESP_RETURN_ON_FALSE(available, ESP_ERR_INVALID_STATE, TAG, "当前没有可展示的配网 Portal");
    return ESP_OK;
}
...
{
    ESP_RETURN_ON_FALSE(out_has_saved_config != nullptr,
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "配置状态输出指针为空");
    taskENTER_CRITICAL(&s_public_state.lock);
 ...
 */
esp_err_t network_manager_set_notify_callback_borrow(network_manager_notify_cb_t callback,
                                                     void                       *ctx)
{
    ESP_RETURN_ON_FALSE(network_manager_is_initialized(),
                        ESP_ERR_INVALID_STATE,
...
{
    ESP_RETURN_ON_ERROR(network_manager_internal_request_start_portal(),
                        TAG,
                        "提交启动配网 Portal 命令失败");
    return ESP_OK;
}
