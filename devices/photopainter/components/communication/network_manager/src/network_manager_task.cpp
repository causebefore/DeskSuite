...
static_assert(std::is_trivially_copyable_v<network_command_t>,
              "Network Manager 命令必须可按值复制");
...
static esp_err_t network_manager_on_portal_submission(const connect_portal_submission_t *submission,
                                                      void                              *ctx);
...
static bool network_manager_config_is_valid(const network_manager_config_t *config)
{
    return config != nullptr && config->ssid[0] != '\0'
           ...
           && network_manager_config_string_is_terminated(config->password,
                                                          sizeof(config->password))
           && network_manager_config_string_is_terminated(config->service_url,
                                                          sizeof(config->service_url))
           && network_manager_config_string_is_terminated(config->device_token,
...
static void network_manager_publish(network_manager_state_t state, esp_err_t error)
{
    ...
    network_manager_internal_publish_status_copy(&s_runtime.working_status,
                                                 s_runtime.has_saved_config);
}
...
static esp_err_t network_manager_post_command(const network_command_t *command)
{
    ...
    ESP_RETURN_ON_FALSE(s_runtime.command_queue != nullptr,
                        ESP_ERR_INVALID_STATE,
                        TAG,
                        "网络命令队列尚未初始化");
    uint32_t current_session_id;
    ESP_RETURN_ON_FALSE(network_manager_get_lifecycle(&current_session_id)
                            == NETWORK_LIFECYCLE_RUNNING,
                        ESP_ERR_INVALID_STATE,
                        TAG,
                        "当前没有可接收命令的网络会话");
    ...
    ESP_RETURN_ON_FALSE(queued.session_id == current_session_id,
                        ESP_ERR_INVALID_STATE,
                        TAG,
                        "网络命令所属会话已失效");
 ...
 */
static bool network_manager_take_pending_link_event(uint32_t              expected_session_id,
                                                    connect_link_event_t *out)
{
    if (out == nullptr)
    {
        return false;
    ...
    const bool pending =
        s_runtime.link_event_pending && s_runtime.pending_link_session_id == expected_session_id;
...
static void network_manager_complete_online(const connect_link_info_t *link)
{
    if (s_runtime.connection_source == NETWORK_CONNECTION_CANDIDATE)
    {
        ...
        const esp_err_t save_error =
            s_runtime.config_store.save_config_borrow(&s_runtime.pending_config,
                                                      s_runtime.config_store.ctx);
    ...
    {
        const esp_err_t save_error =
            s_runtime.config_store.save_config_borrow(&s_runtime.active_config,
                                                      s_runtime.config_store.ctx);
        if (save_error != ESP_OK)
        {
            const esp_err_t stop_error = connect_stop();
            network_manager_publish(NETWORK_STATE_ERROR,
                                    stop_error == ESP_OK ? save_error : stop_error);
            return;
        }
        s_runtime.has_saved_config = true;
    }
    ...
    ESP_LOGI(TAG,
             "Wi-Fi 已联网: SSID=%s, IP=%s, RSSI=%d dBm",
             ssid,
             link->ip,
             (int) link->rssi_dbm);
}
...
static void network_manager_connect_station(void)
{
    ...
    s_runtime.connect_deadline =
        xTaskGetTickCount() + pdMS_TO_TICKS(NETWORK_STA_CONNECT_TIMEOUT_MS);
...
{
    if (submission == nullptr || s_runtime.state != NETWORK_STATE_PROVISIONING
        || !s_runtime.portal_info.active)
    {
    ...
    memset(&s_runtime.pending_config, 0, sizeof(s_runtime.pending_config));
    utils_copy_string(s_runtime.pending_config.ssid,
                      sizeof(s_runtime.pending_config.ssid),
                      submission->ssid);
...
static void network_manager_handle_portal_activity(void)
{
    if (!s_runtime.portal_info.active
        || (s_runtime.state != NETWORK_STATE_PROVISIONING
            && s_runtime.state != NETWORK_STATE_VALIDATING))
    {
        return;
    }
    ++s_runtime.working_status.portal_activity_sequence;
...
static void network_manager_finish_session(void)
{
    ...
    memset(&s_runtime.pending_config, 0, sizeof(s_runtime.pending_config));
    network_manager_clear_portal_info();
    network_manager_publish(stop_err == ESP_OK ? NETWORK_STATE_STOPPED : NETWORK_STATE_ERROR,
                            stop_err);
    if (stop_err == ESP_OK)
    {
...
static void network_manager_task(void *arg)
{
    ...
    while (true)
    {
        ...
        network_command_t command;
        if (xQueueReceive(s_runtime.command_queue, &command, network_manager_get_wait_ticks())
            != pdTRUE)
        {
            if (!network_manager_session_is_running(task_session_id))
            {
            ...
            else if (s_runtime.state == NETWORK_STATE_CONNECTING
                     || s_runtime.state == NETWORK_STATE_VALIDATING)
            {
                connect_link_info_t link      = {};
                const esp_err_t     query_err = connect_get_link_snapshot_copy(&link);
                ...
                {
                    ESP_LOGW(TAG,
                             "等待 Wi-Fi 获取 IP 超时：%u 毫秒",
                             NETWORK_STA_CONNECT_TIMEOUT_MS);
                    const esp_err_t failure_error =
                        query_err != ESP_OK ? query_err : ESP_ERR_TIMEOUT;
                    if (s_runtime.state == NETWORK_STATE_VALIDATING)
                    {
                    ...
                    {
                        const esp_err_t stop_error = connect_stop();
                        network_manager_schedule_retry(stop_error == ESP_OK ? failure_error
                                                                            : stop_error);
                    }
                }
            }
        ...
        switch (command.type)
        {
            case NETWORK_COMMAND_BOOT: {
                ...
                network_manager_clear_portal_info();
                esp_err_t err = s_runtime.config_store.load_config_copy(&s_runtime.active_config,
                                                                        s_runtime.config_store.ctx);
                ...
                {
                    ESP_LOGW(TAG,
                             "网络配置损坏或版本不兼容，清除该配置并重新配网：%s",
                             esp_err_to_name(err));
                    const esp_err_t erase_err =
                        s_runtime.config_store.erase_config(s_runtime.config_store.ctx);
 ...
 */
esp_err_t
    network_manager_internal_task_init_borrow(const network_manager_config_store_t *config_store)
{
    ESP_RETURN_ON_FALSE(config_store != nullptr && config_store->load_config_copy != nullptr
                            && config_store->save_config_borrow != nullptr
                            && config_store->erase_config != nullptr,
                        ESP_ERR_INVALID_ARG,
                        TAG,
                        "网络配置持久化回调不完整");
    ...
    ESP_GOTO_ON_FALSE(s_runtime.command_queue != nullptr,
                      ESP_ERR_NO_MEM,
                      cleanup,
                      TAG,
                      "创建网络管理命令队列失败");
...
static esp_err_t network_manager_rollback_start(esp_err_t error)
{
    ...
    s_runtime.lifecycle =
        cleanup_err == ESP_OK ? NETWORK_LIFECYCLE_INITIALIZED : NETWORK_LIFECYCLE_CLEANUP_FAILED;
    taskEXIT_CRITICAL(&s_runtime.lifecycle_lock);
    return error;
}
...
esp_err_t network_manager_internal_task_stop(void)
{
    ...
    TaskHandle_t task       = nullptr;
    taskENTER_CRITICAL(&s_runtime.lifecycle_lock);
    const bool can_stop = (s_runtime.lifecycle == NETWORK_LIFECYCLE_RUNNING
                           || s_runtime.lifecycle == NETWORK_LIFECYCLE_STOPPING)
                          && s_runtime.manager_task != nullptr
                          && s_runtime.manager_task != xTaskGetCurrentTaskHandle();
    if (can_stop)
    {
        s_runtime.lifecycle = NETWORK_LIFECYCLE_STOPPING;
        session_id          = s_runtime.session_id;
    ...
    ESP_RETURN_ON_FALSE(
        xSemaphoreTake(s_runtime.task_stopped, pdMS_TO_TICKS(NETWORK_MANAGER_STOP_TIMEOUT_MS))
            == pdTRUE,
        ESP_ERR_TIMEOUT,
        TAG,
    ...
    taskENTER_CRITICAL(&s_runtime.lifecycle_lock);
    s_runtime.manager_task = nullptr;
    s_runtime.lifecycle    = s_runtime.stop_result == ESP_OK ? NETWORK_LIFECYCLE_INITIALIZED
                                                             : NETWORK_LIFECYCLE_CLEANUP_FAILED;
    taskEXIT_CRITICAL(&s_runtime.lifecycle_lock);
