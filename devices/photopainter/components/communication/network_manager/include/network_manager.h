 ...
 */
    typedef esp_err_t (*network_manager_config_load_cb_t)(network_manager_config_t *out_config,
                                                          void                     *ctx);
 ...
 */
    typedef esp_err_t (*network_manager_config_save_cb_t)(const network_manager_config_t *config,
                                                          void                           *ctx);
 ...
 *
 * 仅在状态为 NETWORK_STATE_PROVISIONING 或 NETWORK_STATE_VALIDATING 时有效，
 * 大型二维码载荷只在调用本函数时复制。
 *
 * @param[out] out_info Portal 展示信息输出指针
 * @return ESP_OK 成功；ESP_ERR_INVALID_ARG 参数无效；
 *         ESP_ERR_INVALID_STATE 当前没有可展示的 Portal
 */
