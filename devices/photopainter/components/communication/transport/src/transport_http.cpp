...
static long long transport_http_elapsed_ms(int64_t started_us, int64_t finished_us)
{
    return started_us > 0 && finished_us >= started_us
               ? static_cast<long long>((finished_us - started_us) / 1000LL)
               : -1LL;
}

/** @brief 输出缓冲式 HTTP 请求的分阶段耗时 */
static void transport_http_log_timing(const char *url, int status, esp_err_t error,
                                      const response_buffer_t &buffer, int64_t returned_us)
{
                                        ...
                                        transport_http_response_t      *response)
{
    ...
    const size_t capacity =
        (request->max_response_bytes > 0 ? request->max_response_bytes : DEFAULT_RESPONSE_LIMIT)
        + 1;
    ...
    esp_err_t err = esp_http_client_set_method(client.get(), to_idf_method(request->method));
    for (size_t i = 0; err == ESP_OK && i < request->header_count; ++i)
    {
        ...
        err = esp_http_client_set_header(client.get(),
...
{
    if (request == nullptr || request->url == nullptr || request->upload_data == nullptr
        || request->upload_len == 0 || request->on_response_data == nullptr || result == nullptr)
    {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    while (err == ESP_OK)
    {
        ...
        const int received = esp_http_client_read(client.get(),
                                         ...
                                         transport_http_download_result_t        *result)
{
    ...
    if (err == ESP_OK)
    {
        result->content_length = esp_http_client_fetch_headers(client.get());
        result->status_code    = esp_http_client_get_status_code(client.get());
        if (result->status_code < 200 || result->status_code >= 300)
        {
    ...
    if (err == ESP_OK)
    {
        buffer.reset(static_cast<uint8_t *>(
            heap_caps_malloc(read_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)));
        if (buffer == nullptr)
        {
            err = ESP_ERR_NO_MEM;
        }
